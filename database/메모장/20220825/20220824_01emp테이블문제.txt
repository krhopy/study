...
-- 현재 연봉 구하기 (단, 커미션이 없으면 null값에 0 넣어 계산)
...
-- 현재연봉 + 인상된 금액 = 최종연봉 구하기
select ename, hiredate, sal
       ,sal*12+nvl(comm, 0) as 현재연봉
       ,(sal*12+nvl(comm, 0))*0.1 as 인상된금액
       ,(sal*12+nvl(comm, 0))+((sal*12+nvl(comm, 0))*0.) as 최종연봉    --(현재연봉)+(인상된금액)
from emp
where trunc((sysdate-hiredate)/365) >= 15;
...
select ename, hiredate, sal
       ,sal*12+nvl(comm, 0) as 현재연봉
       ,(sal*12+nvl(comm, 0))*0.1 as 인상된금액
       ,round((sal*12+nvl(comm, 0))+((sal*12+nvl(comm, 0))*0.)) as 최종연봉    --(현재연봉)+(인상된금액)
from emp
where trunc((sysdate-hiredate)/365) >= 15;
